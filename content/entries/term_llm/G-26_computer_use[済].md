...
**著者の指摘（2026-09-06）**: **パソコン上の操作を何でもできる万能性**が印象的。誰にでも使える機能という点も強い。

- **難点はレイテンシとコスト**。**画面を読ませて判断させる**という動き方そのものが重い
- **不安もある**。「これを読んで操作する」に任せきるのは怖い

**それでも効く場面がはっきりある。**MCP のような連携の口がなくても入っていける**ので、**固有のベンダーのシステム**を少し触りたいときには、**いちばんシンプルな入口**になる。まず AI で使ってみる、という**最初のステップとしては良い**。入力情報を与えておけば、後はいい感じにしてくれる再現性もある。

**ただし本命ではない**、というのが著者の結論。**今後どこまで使うかは、後々の展開を見て考えるべき**。（I-1 MCP、G-49 AI エージェントと接続）
