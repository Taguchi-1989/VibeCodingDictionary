...
- 数字を変えると何がどう変わるのか分かりません
- いくつにすればよいのか、基準がありません
- そもそも触れる画面が見当たりません
...
- 🙂 第一印象: 自由度をどこまで許すかの調整だと分かります
- 👍 良い点: 仕組みとして知っておく価値はあります
- 👎 ダメな点: 実際に触る機会は減ってきました
- 👥 誰向けか: アプリを提供する側の人向けです
...
**著者の指摘（2026-09-06）**: **どれだけ自由度を許すか**の調整という理解。Top-p などと並べて語られることが多い。

**ただし、いま実際にここを触る機会は少なくなってきている**というのが実感。**アプリケーションを提供する側でないユーザーは、知らなくても困らない**かもしれない。それでも**仕組みとして「許容の幅を決められる」ことを知っておく価値はある**。

**難しいのは中間の値。**0 や 1 のような端は意味が分かるが、**中間として何を持たせるべきかの判断が微妙**で、そこが実務でのつまずきになる。

（前に話された「**決定論に寄せる**」という設計方針と直結する項目。G-50 Evals や J-133 の再現性の話と接続させたい。）
