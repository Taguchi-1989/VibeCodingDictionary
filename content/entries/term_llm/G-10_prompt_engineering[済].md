...
- **差し色: 第 1 層「プロンプト」をベタ塗り＋枠を太く。** 内側の「LLM」は差し色の薄いトーン、外側の「コンテキスト」「ループ」「ガード」「ルーティング」「ハーネス」はすべて灰色の破線枠のみ
- 伝えたい一言: 「1 回の呼び出しの、言い方だけ」
- 注意: 既存の最終ポンチ絵（G-10.webp）はこのミニ図を含まないため、**再生成が必要**。メイン図の構図自体は現行を踏襲する
