...
コード生成・ファイル編集・テスト実行・Git 操作をひとつながりでこなします。出発点はターミナルですが、いまはデスクトップアプリ・ブラウザ・IDE 拡張からも同じものを使えます。
...
- **2026-09-05 の当て直し**: 「CLI 上で動く」だけではなくなりました。ターミナルの CLI に加えて、**デスクトップアプリ（Mac / Windows）・ブラウザ（claude.ai/code）・IDE 拡張（VS Code、JetBrains）**から使えます。「ターミナルの道具だから自分には関係ない」と読者に思わせないよう、誌面本文もそこを書き換えています
