...
- **G-37 プロンプトインジェクション** — 外部の文章に仕込んだ指示で AI を乗っ取る攻撃
...
- **G-37 プロンプトインジェクション**：G-37 は Cursor Composer を D-35 へ移した際の欠番です。「G-30 台＝連携・制御」の棚に他の空きがないため再利用しました（欠番の詰め直しではなく、空き番号への新規追加）。番号を動かしたくない場合は著者判断で差し替えてください
