...
## M 差分（2026-08-10）— 収録語の抜け監査による追加 34 件

*[docs/design_improvement_proposal_2026-08.md](../docs/design_improvement_proposal_2026-08.md) §4「収録語の抜け監査」で洗い出した候補を、空き番号に割り当てて台帳へ登録しました。仮 ID から変更した箇所は末尾の「ID 調整メモ」に理由を書いています。*

### ☆☆☆ 明らかな抜け（14 件＋復活 1 件）

- **G-49 AI エージェント** — 目標を渡すと自分で手順を決めて動く AI システムの総称
- **F-213 API** — ソフト同士が決まった形でやり取りするための窓口
- **F-214 API キー** — API を使う人を識別する秘密の文字列。漏えい対策が要る
- **F-45 デプロイ** — 作ったものを本番環境に置いて公開する作業
- **F-46 デバッグ** — 不具合の原因を特定して直していく作業
- **F-18 フレームワーク／ライブラリ** — 土台として使う枠組みと、部品として呼ぶ道具箱の違い
- **F-47 フロントエンド／バックエンド** — 画面側と裏側という開発の役割分担
- **J-34 マルチモーダル** — 文字・画像・音声・動画をまとめて扱える性質
- **J-35 スケーリング則** — 計算量・データ量・パラメータ数を増やすほど性能が伸びる経験則
- **G-27 プロンプトインジェクション** — 外部の文章に仕込んだ指示で AI を乗っ取る攻撃
- **E-28 ARC-AGI** — 見たことのない規則をその場で推論できるかを測るベンチマーク
- **B-34 NotebookLM** — 手元の資料だけを根拠に答えさせる Google のノートツール
- **B-35 Gemini CLI** — ターミナルで動く Google 製のオープンソース AI エージェント
- **D-59 Midjourney** — 作風の強さで知られる画像生成サービス
- **F-43 テスト**（既存）— archived だった既存エントリを復活。新規採番なし

### ☆☆ 検討枠（20 件）

- **B-36 Lovable** — 会話だけでアプリ一式を作るバイブコーディング系サービス
- **B-37 Replit** — ブラウザだけで書いて動かして公開できる開発環境
- **B-38 Manus** — 汎用タスクを任せられる自律型 AI エージェント
- **B-39 Google AI Studio** — Gemini と API キーを無料で試せる入口
- **B-42 Discord** — AI コミュニティの実際の集合場所になっているチャットサービス
- **B-43 n8n・Dify** — 非エンジニアでも組める AI ワークフロー自動化ツール
- **C-16 DeepSeek** — 重みを公開する中国・杭州の AI 開発企業（モデルは D-46 / D-47）
- **C-17 Sakana AI** — 自然界の知能に着想を得た日本発の AI スタートアップ
- **D-61 AlphaFold** — タンパク質の立体構造を予測しノーベル賞につながったモデル
- **G-24 Temperature** — 出力のばらつきを決める設定値
- **G-25 AI のメモリ機能** — 過去の会話から好みや前提を覚えておく仕組み
- **G-26 Computer Use** — 画面を見てマウスとキーボードを操作させる使い方
- **J-36 蒸留** — 大きいモデルの出力を教師にして小さいモデルを鍛える手法
- **J-57 RLHF・アラインメント** — 人間の評価で学習させ、AI の振る舞いを目的に合わせる取り組み
- **J-115 NPU・AI PC** — PC 内で AI 処理を担う専用チップと、それを積んだ PC
- **J-116 TPU** — Google が自社開発した AI 計算専用チップ
- **J-117 データセンターと電力** — AI の計算を支える施設と、その電力消費の議論
- **J-5 世界モデル** — 世界の動きを内部でシミュレーションして予測する AI
- **J-38 ムーアの法則** — 半導体の集積度が一定周期で倍増するという経験則
- **H-64 DeepSeek ショック** — 2025-01 に株式市場まで動かした低コスト AI の衝撃

## N 差分（2026-09-02）— 「〜エンジニアリング」系の整理

*「プロンプト／コンテキストはあるのに、ハーネス・ループ・グラフは？」という著者の指摘から、5 語を「同じ入れ子の地図の、どこを担当しているか」で描き分ける方針に切り替えました。作図仕様は [docs/scope_map_figure_spec.md](../docs/scope_map_figure_spec.md)。*

- **J-118 グラフエンジニアリング**（新規）— ノードとエッジで処理を配線する工学。ループ層の中の「繋ぎ方」を担う
- **J-103 ループエンジニアリング**（既存）— reader_level 6 → 5 へ昇格し、刊行スコープに入れました
- **J-106 ハーネスエンジニアリング**（既存）— reader_level 6 → 5 へ昇格。5 兄弟の共通ベース図（スコープ地図）の親 figure を担います
- G-10 プロンプトエンジニアリング / G-11 コンテキストエンジニアリングは内容そのままで、メイン図に共通ミニ図を描き足す指示のみ追加
...
|---|---|
| G-10 プロンプト | 第 1 層のみ |
| G-11 コンテキスト | 第 1〜2 層（プロンプトを内包） |
| J-103 ループ | 第 3 層 |
| J-118 グラフ | 第 3 層の中の「配線」 |
| J-106 ハーネス | 最外枠まで全部 |

---

### ID 調整メモ（提案書の仮 ID からの変更）

- **F-43 テスト**：提案書は「欠番」と書いていましたが、実際には v2 テンプレで書き上げ済みのエントリが `status: archived` で凍結されていました。新規採番せず、既存ファイルを復活させます
- **E-5 → E-28 ARC-AGI**：E-1〜19 は「コーディング・エンジニアリング」棚。ARC-AGI は推論系のため「E-20〜29 推論・知識」の空き番号へ
- **J-34/J-35 → 据え置き、J-72〜74 → J-115〜117**：提案書の仮 ID のうち J-72〜74 は H100 / Blackwell / RTX シリーズで既に使用済みでした。ハードウェア棚（J-70 台）に空きがないため、末尾の空き番号へ回しています
- **G-27 プロンプトインジェクション**：当初は G-37（Cursor Composer を D-35 へ移した際の欠番）に置きましたが、著者判断で **G-27 へ移動**しました。G-37 は引き続き欠番のまま残します。今回追加した G-24 Temperature / G-25 メモリ機能 / G-26 Computer Use と同じ並びに入ります

---

## O 差分（2026-09-05）— LLM 活用の「いま熱いところ」14 件

*著者の指示（2026-09-05）で、**歴史的な語ではなく 2026 年の実務でいま効いている語**のうち、本編に書き漏れているものを監査して追加しました。ブリーフ（押さえる事実・出典 URL・書かないこと）は [ledgers/stage11_briefs_2026-09.md](stage11_briefs_2026-09.md)。すべて 2026-09-05 に web で確認しています。元ネタは [ledgers/loop_engineering_landscape_2026.md](loop_engineering_landscape_2026.md) の、台帳止まりになっていた概念群です。*

### 新設サブ棚

- **G-50〜59 評価・検証** — G-40 台「運用・品質」が満杯のため新設。E 章（他人が作ったベンチマーク）に対して、**自分の仕事に合わせた物差しを作る側**の棚
- **G-60〜69 検索・知識連携** — G-10 台「エンジニアリング技法」が満杯のため新設。G-15 RAG の深掘り群をここに集める

### 追加 14 件

#### 評価・検証（G-50〜51）

- **G-50 Evals** — 自分の仕事に合わせた評価をつくり、繰り返し測るしくみ。著者の「チェックボックス検収」構想の着地点
- **G-51 LLM-as-a-judge** — AI の出力を別の AI に採点させるやり方。審判自体を人のラベルと突き合わせて検証する

#### 検索・知識連携（G-60〜64）

- **G-60 GraphRAG** — 知識グラフに作り替えてから探す RAG。全体を見渡す質問に効く。本家はメンテナンスモードで、考え方が後続に受け継がれている
- **G-61 ナレッジグラフ** — 「もの」を点、「関係」を線で表した知識の地図。**J-118 グラフエンジニアリング（処理の配線）とは載っているものが違う**
- **G-62 チャンク分割** — 長い資料を検索できる大きさに切る前処理。切り方が回答品質をほぼ決める
- **G-63 ハイブリッド検索** — キーワード検索とベクトル検索を両方走らせて順位で合成する。型番・エラーコードはベクトルだけでは拾えない
- **G-64 リランキング** — 広く拾った候補を別のモデルで並べ直し、上位だけを LLM に渡す工程

#### ループ工学クラスタの続き（J-128〜134）

- **J-128 ワークフローとエージェント** — **そもそも自律させるか**の分岐。手順を人が決めるか AI が決めるか。J-103 は「自律させると決めた後の回し方」で分担
- **J-129 コンテキスト腐敗** — context rot / lost in the middle。台帳の「敵②文脈が溢れる」の実体
- **J-130 推論時スケーリング** — 答えるときに計算を使って性能を上げる。G-14 思考モデル・G-9 Effort Level の裏側の原理
- **J-131 RLVR** — 検証可能な報酬で鍛える強化学習。J-57 RLHF との「教師の違い」が軸
- **J-132 A2A** — エージェント同士を繋ぐプロトコル。MCP（縦・道具への接続）に対して横方向。競合ではなく同じ財団の下に並ぶ
- **J-133 オブザーバビリティ** — 何をどう処理したかを後から追える状態。OpenTelemetry の GenAI 規約に収束、ただしまだ experimental
- **J-134 Human-in-the-loop** — 承認ゲートを 3 段階（自動承認／通知／停止）で置く設計。著者の「①方向づけ・②承認・③検収」の②

### 監査で見えた穴（記録）

grep で実在確認した結果、追加前は以下がいずれも本編に **0 件**でした。E 章がベンチマークの読み方だけを扱い、**自分の物差しを作る話が丸ごと無かった**のがいちばん大きな穴です。

`LLM-as-a-judge` / `GraphRAG` / `ナレッジグラフ` / `リランキング` / `ハイブリッド検索` / `オブザーバビリティ` / `context rot` / `A2A` / `RLVR` / `推論時スケーリング` / `human-in-the-loop` / `DAG` / `ステートマシン`

このうち DAG・ステートマシンは J-118 グラフエンジニアリング・J-104 ReAct ループで実質的に説明されているため、今回は立てていません。
