# 次セッションへの引き継ぎ（2026-08-23 v13 更新）
...
2. **⚠️ 警告 139 件の手当て**：内訳は主に `reader_level` の書式（`"2-3"` のクォート付き・幅 2 段以上）と強い断定語（「最新」など）。機械的に直せるものが多いので、一括処理を検討する価値があります
3. **CSV と実ファイルの status 乖離**（前セッションから継続。`sync_entries_csv.py` は path 列しか同期しない）
